...
Of course, we would love to hear about it, to help us improve {link-kotlinfixture}
...
you can also create your own by adding extension functions to `link:src/main/kotlin/com/detomarco/kotlinfixture/config/Generator.kt[Generator]`.

`link:src/main/kotlin/com/detomarco/kotlinfixture/config/Generator.kt[Generator]`
...
The core engine of {link-kotlinfixture} is based on a chain of `{url-resolver}[Resolver]` instances
which are queried in sequence. We ask each `{url-resolver}[Resolver]`
if it handles the current input object, and it returns either a generated fixture or `link:src/main/kotlin/com/detomarco/kotlinfixture/Unresolved.kt[Unresolved.Unhandled]`.

NOTE: We use link:src/main/kotlin/com/detomarco/kotlinfixture/Unresolved.kt[Unresolved] as nulls are valid fixtures.
...
link:src/main/kotlin/com/detomarco/kotlinfixture/decorator/logging/LoggingDecorator.kt[logging]
to link:src/main/kotlin/com/detomarco/kotlinfixture/decorator/recursion/RecursionDecorator.kt[recursion].
...
For convenience, we add an extension function to the link:src/main/kotlin/com/detomarco/kotlinfixture/config/ConfigurationBuilder.kt[ConfigurationBuilder].
For example, see link:src/main/kotlin/com/detomarco/kotlinfixture/decorator/logging/LoggingConfiguration.kt[LoggingConfiguration].
